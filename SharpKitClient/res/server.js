...
function server() {
    ...
    this._onReceive = function (str) {
        ...
        if (obj._cstype) {
            var inst = new SharpKitLib.entity[obj._cstype]();
            if (!inst) {
                alert("Can't find class SharpKitLib.entity." + obj._cstype);
            }
            for (var name in obj) {
                var val = obj[name];
            ...
            var fnName = obj._cstype.replace("Data", '');
            var fn = $mvcHlp.prototype[fnName];
            fn.call($mvcHlp, inst);
        }
